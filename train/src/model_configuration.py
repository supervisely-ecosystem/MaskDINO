...
class IterationHook(HookBase):
    def before_step(self):
        ...
        if current_iter == 0:
            self.trainer.sly_logger.train_started(
                total_epochs=self.trainer.cfg.SOLVER.MAX_ITER
            )
...
def configure_trainer(train):
    ...
    # cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES = len(train.classes)
    ...
    cfg.SOLVER.STEPS = tuple(
        range(
            hyperparameters["lr_decay_frequency"],
            cfg.SOLVER.MAX_ITER,
            hyperparameters["lr_decay_frequency"],
        )
    )
